...
FLUSH PRIVILEGES;
USE pftpd;
CREATE TABLE ftpd (User varchar(20) NOT NULL default '',
...
# Set passive ports
echo "40000 45000" | sudo tee /etc/pure-ftpd/conf/PassivePortRange > /dev/null
...
sudo sed -i "/CertFileAndKey/c\CertFileAndKey		\"/etc/letsencrypt/live/$hostname/fullchain.pem\"	\"/etc/letsencrypt/live/$hostname/privkey.pem\"" /etc/pure-ftpd/pure-ftpd.conf
...
#################################
## Enable firewall and restart ##
#################################
echo -e "You dungoofed! The system will self destruct!"
sudo csf -e && sudo reboot  